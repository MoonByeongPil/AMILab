...
#이해안됨!!!!! a11과 a22는 같은 위치에서 같은 1이라는 값을 갖지 않나?
    cos_angle = a12/torch.sqrt(torch.mul(a11,a22))   #cos각을 값으로 갖는 행렬 cos 공식과 동일. 메모장에 기록.
...
def calc_loss_d_refined_mask(output, y, z_refined):
    ...
    matrix_max_y = torch.transpose(torch.reshape(max_y.repeat(multiply), [ multiply[0], max_y.size()[0]]))   #tile=max_y를 복붙하는데 256*256 길이로 복붙. 각 픽셀마다 max_y 벡터 존재.
    ...
    matrix_max = torch.transpose(torch.reshape(output_max.repeat(multiply), [ multiply[0], output_max.size()[0]]))   #위 행렬을 256*256
    ...
    matrix_min = torch.transpose(torch.reshape(output_min.repeat(multiply), [ multiply[0], output_min.size()[0]]))
